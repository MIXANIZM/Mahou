...
The first PR #18 candidate at `5527f662cb844ba90d264f5b93cb27f8334bf295` is rejected as `USER_SMOKE_FAILED / STARTUP_HANG_HIGH_CPU`; it must not be retried. `AGZ-MAH-0020` replaces the unbounded dictionary startup path. Replacement exact-head CI and provenance pass and are recorded in Draft PR #18; the two-stage physical-Windows smoke remains required and no physical verification is claimed.
